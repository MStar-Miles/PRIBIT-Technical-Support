...
- [Installation Steps](#installation-steps)
  - [1. PCC Package 압축 해제](#1-pcc-package-압축-해제)
  - [2. PCC 설치](#2-pcc-설치)
    - [설치 스크립트 install\_offline.sh 실행](#설치-스크립트-install_offlinesh-실행)
    - [PCC 가 사용할 Database 설치](#pcc-가-사용할-database-설치)
    - [PCC Console 에 접근 허용할 IP Address 설정](#pcc-console-에-접근-허용할-ip-address-설정)
    - [PCC 에서 사용하는 라이브러리 설치](#pcc-에서-사용하는-라이브러리-설치)
    - [PCC 설치 마무리](#pcc-설치-마무리)
  - [3. PCC Process 확인](#3-pcc-process-확인)
  - [4. 초기 설정 및 라이선스 적용](#4-초기-설정-및-라이선스-적용)
- [설치 스크립트 기준 패키지 / 프로세스](#설치-스크립트-기준-패키지--프로세스)
  - [설치하는 패키지 리스트](#설치하는-패키지-리스트)
  - [실행시키는 프로그램 리스트](#실행시키는-프로그램-리스트)
...
- net-tools
- rdate
- vim-nox (Ubuntu 24.04)
- rsyslog (Ubuntu 24.04)
- openjdk-17-jre
- build-essential
- libpcre3-dev
- zlib1g-dev
...
### 실행시키는 프로그램 리스트
- mariadb.service (MariaDB DBMS)
- nginx.service (Nginx 웹서버)
- redis-server.service (Redis 서버)
- connect-controller.service (Controller RPC 서비스)
- connect-controller-web.service (Controller Web 서비스)
- connect-controller-api.service (Controller API 서비스)
- connect-controller-check.service (Controller Check 서비스)
- java -jar /opt/connect/controller/util/controller-util-1.0.jar (설정 및 레거시 데이터 처리)
- thirdparty-conf-set (설정 스크립트 바이너리)
- pribit_encoder (로그/설정 파일 인코더)
